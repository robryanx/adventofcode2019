...
func run_combination(opcodes []int, phases []int) int {
	...
	for i := range phases {
		...
		if i == 0 {
			go intcode.Run_computer(i, run_opcodes, channel[len(phases)-1], channel[i], exit[i], false)
		} else {
			go intcode.Run_computer(i, run_opcodes, channel[i-1], channel[i], exit[i], false)
		}
	}
